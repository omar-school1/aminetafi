...
// دمج بيانات QCM من الملفات المنفصلة مع بيانات الدروس
function integrateQCMData() {
    try {
        console.log('🔄 بدء دمج بيانات QCM...');
        
        // خريطة ربط أسماء المستويات
        const levelMapping = {
            'common-core': qcmCommonCore,
            'first-bac': qcmFirstBac,
            'second-bac': qcmSecondBac
        };
        
        // دمج البيانات لكل مستوى
        Object.keys(levelMapping).forEach(levelKey => {
            const qcmData = levelMapping[levelKey];
            if (qcmData && qcmData.subjects && levelSubjects[levelKey]) {
                
                // دمج بيانات QCM مع كل مادة
                qcmData.subjects.forEach(qcmSubject => {
                    // البحث عن المادة المقابلة في levelSubjects
                    const subjectIndex = levelSubjects[levelKey].findIndex(subject => 
                        subject.name === qcmSubject.name
                    );
                    
                    if (subjectIndex !== -1) {
                        // دمج الدروس مع بيانات QCM
                        if (qcmSubject.lessons) {
                            qcmSubject.lessons.forEach((qcmLesson, qcmLessonIndex) => {
                                // البحث عن الدرس المقابل في levelSubjects
                                const lessonIndex = levelSubjects[levelKey][subjectIndex].lessons.findIndex(lesson => 
                                    lesson.title === qcmLesson.title
                                );
                                
                                if (lessonIndex !== -1 && qcmLesson.quiz) {
                                    // إضافة بيانات QCM إلى الدرس
                                    levelSubjects[levelKey][subjectIndex].lessons[lessonIndex].quiz = qcmLesson.quiz;
                                    console.log(`✅ تم دمج QCM للدرس: ${qcmLesson.title} في ${qcmSubject.name} - ${levelKey}`);
                                }
                            });
                        }
                    }
        ...
        console.log('✅ تم دمج بيانات QCM بنجاح');
        
    } catch (error) {
        console.error('❌ خطأ في دمج بيانات QCM:', error);
    }
}

// تشغيل دمج البيانات عند تحميل الصفحة
document.addEventListener('DOMContentLoaded', function() {
    // دمج بيانات QCM مباشرة بعد تحميل الصفحة
    integrateQCMData();
});
