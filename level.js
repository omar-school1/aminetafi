ي// Level page JavaScript functionality
...
function downloadLesson(lessonTitle, subjectName) {
    // الحصول على المستوى الحالي من URL
    const urlParams = new URLSearchParams(window.location.search);
    const currentLevel = urlParams.get('level') || 'common-core';
    
    // البحث عن الدرس في البيانات
    const subjects = levelSubjects[currentLevel];
    const subject = subjects.find(s => s.name === subjectName);
    
    if (subject) {
        const lesson = subject.lessons.find(l => l.title === lessonTitle);
        
        if (lesson && lesson.downloadUrl) {
            // إذا كان هناك رابط تحميل محدد للدرس
            window.open(lesson.downloadUrl, '_blank');
            return;
        }
    }
    
    // إذا لم يوجد رابط محدد، البحث في ملفات GitHub
    const levelMap = {
        'common-core': 'tronc_commun',
        'first-bac': 'premiere_bac', 
        'second-bac': 'deuxieme_bac'
    };
    ...
    const subjectMap = {
        ...
        'الفيزياء': 'physique',
        'علوم الحياة': 'biologie',
        'الفلسفة': 'philosophie',
        'الاجتماعيات': 'social'
    };
    
    const levelFolder = levelMap[currentLevel] || 'tronc_commun';
    const subjectFolder = subjectMap[subjectName] || 'arabe';
    
    // تكوين رابط تحميل من GitHub للملفات المرفوعة
    const fileName = `${lessonTitle}.pdf`; // افتراض أن الملف PDF
    const githubUrl = `files/lessons/${levelFolder}/${subjectFolder}/${fileName}`;
    
    // محاولة تحميل الملف
    fetch(githubUrl)
        .then(response => {
            if (response.ok) {
                // إذا وجد الملف، فتح رابط التحميل
                window.open(githubUrl, '_blank');
            } else {
                // إذا لم يوجد الملف، عرض رسالة
                alert(`عذراً، ملف الدرس "${lessonTitle}" غير متوفر حالياً.\nيمكنك تحميله من خلال لوحة التحكم.`);
            }
        })
        .catch(error => {
            // في حالة عدم توفر الملف أو خطأ في الشبكة
            alert(`عذراً، ملف الدرس "${lessonTitle}" غير متوفر حالياً.\nيمكنك تحميله من خلال لوحة التحكم.`);
        });
}
